...
# // return true, because 'cd' is in the wordlist
...
class StreamChecker:
    ...
    def query(self, letter: str) -> bool:
        ...
        for last_node in self.last_nodes:
            if letter in last_node:
                new_last_nodes.append(last_node[letter])
        if letter in self.t:
            new_last_nodes.append(self.t[letter])
        self.last_nodes = new_last_nodes
        return any('#' in node for node in self.last_nodes)
